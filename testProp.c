...
int main(int argc, char* argv[]) {
	...
	fwrite(tracesData, 1, curTraceNumber*ns*sizeof(float), output_file);
 	...
 	//sleep(3);
        ...
        sprintf(fileName, "propT150CBordAbsDif/retroPTest_%d.bin", shotNumber);                     
        
	localImage = (float *) malloc(localXi*localYi*sZi*sizeof(float));
 
        propagation( &(vel[localGxMinC*Ybi*Zbi + localGyMinC*Zbi]), localXi, localYi, Yi, Zi, sZi, sdX, sdY, sdZ, tr, ns, sdt, trc, Ntr, 50, fileName, bw, my_rank, shotNumber, 1, divisor, sDivisor, sDivY, sDivZ, localImage, sTi);
        
	//propagation( &(vel[localGxMinC*Ybi*Zbi + localGyMinC*Zbi]), localXi, localYi, Yi, Zi, sZi, sdX, sdY, sdZ, tracesData, ns, sdt, tracesC, curTraceNumber, 100, fileName, bw, my_rank, shotNumber, -1, divisor, sDivisor, sDivY, sDivZ, localImage, sTi); 
...
void propagation(float *vel, unsigned int localXi, unsigned int localYi, int Yi, int Zi, int sZi, float sdx, float sdy, float sdz, float *tr, unsigned short Ti, float sdt, int *trc, int Ntr, int wri, char *wffn, int bw, int my_rank, unsigned int shotNumber, short pMode, double divisor, double sDivisor, double sDivY, double sDivZ, float *localImage, unsigned short sTi) {
    ...
    /* //Making border to become non-reflexive
    for (xbi = ncx-1; xbi < bw; xbi++) {
		borderX[xbi] = pow(((float)xbi-ncx+1)/(bw-ncx+1),2);
		borderX[localXbi-1-xbi] = pow(((float)xbi-ncx+1)/(bw-ncx+1),2);
    }
    ...
    for (ybi = ncy-1; ybi < bw; ybi++) {
		borderY[ybi] = pow(((float)ybi-ncy+1)/(bw-ncy+1),2);
		borderY[localYbi-1-ybi] = pow(((float)ybi-ncy+1)/(bw-ncy+1),2);	
    }
    ...
    for (zbi = ncz-1; zbi < bw; zbi++) {
		borderZ[zbi] = pow(((float)zbi-ncx+1)/(bw-ncz+1),2);
		borderZ[sZbi-1-zbi] = pow(((float)zbi-ncz+1)/(bw-ncz+1),2);
    }
	...
	if (wri != 0) {
		if ((ti+1)%wri == 0 || ti == 10 || ti > sTi-5 || (ti >= 800 && ti < 900)) {
		    if(pMode >= 0)
		    {		
			sprintf(fileName, "propT150CBordAbsDif/diretaTest150_%d.bin", ti);                     
			...
			sprintf(fileName, "retroPT150SAbs/retroPTest_%d.bin", ti);                     
                        FILE* wff2 = fopen(fileName,"wb"); // Wafield record file
			...
			for (xbi = bw; xbi < localXbi-bw; xbi++) {
			   for (ybi = bw; ybi < localYbi-bw; ybi++) {
				//printf("Antes de acessar indice u.cur xbi: %d\tybi: %d\n", xbi, ybi);
			    	if (fwrite(&u.cur[xbi*localYbi*sZbi + ybi*sZbi + bw], sizeof(float), sZi, wff2) != sZi) {
			/*for (xbi = 0; xbi < localXbi; xbi++) {
			   for (ybi = 0; ybi < localYbi; ybi++) {
				if (fwrite(&u.cur[xbi*localYbi*sZbi + ybi*sZbi], sizeof(float), sZbi, wff2) != sZbi) {*/
				...
				//printf("Depoiss de acessar indice u.cur xbi: %d\tybi: %d\n", xbi, ybi);//xbi: 64 ybi:184	
			   }	
		    	}
			printf("Saiu!\n"); 
			/*
    ...
    /*sprintf(fileName, "condImagemT150SAbs/imageTest_%d.bin", shotNumber);
