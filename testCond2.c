...
void propagation(float *vel, unsigned int localXi, unsigned int localYi, int Yi, int Zi, int sZi, float dx, float dy, float dz, float *tr, unsigned short Ti, float dt, int *trc, int Ntr, int wri, char *wffn, int bw, int my_rank, unsigned int shotNumber, short pMode, double divisor, double sDivisor, double sDivY, double sDivZ, float *localImage, unsigned short sTi);
...
int main(int argc, char* argv[]) {
    ...
    localFirstShotNumber = my_rank*localCeilShotNumber + 187;
    shotNumber = 0;
	...
	fwrite(tracesData, 1, curTraceNumber*ns*sizeof(float), output_file);
 	...
 	//sleep(3);
        ...
        sprintf(fileName, "diretaTest3_%d.bin", my_rank);                     
        
	localImage = (float *) malloc(localXi*localYi*sZi*sizeof(float));
 
        propagation( &(vel[localGxMinC*Ybi*Zbi + localGyMinC*Zbi]), localXi, localYi, Yi, Zi, sZi, sdX, sdY, sdZ, tr, ns, sdt, trc, Ntr, 10, fileName, bw, my_rank, shotNumber, 1, divisor, sDivisor, sDivY, sDivZ, localImage, sTi);
 ...
 * bw: border width
 */
void propagation(float *vel, unsigned int localXi, unsigned int localYi, int Yi, int Zi, int sZi, float sdx, float sdy, float sdz, float *tr, unsigned short Ti, float dt, int *trc, int Ntr, int wri, char *wffn, int bw, int my_rank, unsigned int shotNumber, short pMode, double divisor, double sDivisor, double sDivY, double sDivZ, float *localImage, unsigned short sTi) {
    ...
    sprintf(fileName, "imageTest2_%d.bin", shotNumber);
