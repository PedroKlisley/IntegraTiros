...
void propagation(float *vel, unsigned int localXi, unsigned int localYi, int Yi, int Zi, float dx, float dy, float dz, float *tr, unsigned short Ti, float dt, int *trc, int Ntr, int wri, char *wffn, int bw, int my_rank, unsigned int shotNumber, short pMode, double divisor);
...
int main(int argc, char* argv[]) {
    ...
    readVelData(vel, &vMin, &vMax, &sdt, &rdt, &sTi, &divisor, &sdX, &sdY, &sdZ, &rds, &sDivisor, ns, Xi, Yi, Zi, Xbi, Ybi, Zbi, argv[1], bw, dX, dY, dZ, dt, &sXi, &sYi, &sZi, &sXbi, &sYbi, &sZbi);
    ...
    {             
        ...
        getComShotTraces(suFile, traces, tracesData, tracesC, tr, trc, &traceNumber, &curTraceNumber, &localXi, &localYi, &localGxMinC, &localGyMinC, gxMin, gyMin, ntt, ns, TD, sdX, sdY, sdZ, dt);        
	...
	fwrite(tracesData, 1, curTraceNumber*ns*sizeof(float), output_file);
        ...
        propagation( &(vel[localGxMinC*Ybi*Zbi + localGyMinC*Zbi]), localXi, localYi, Yi, Zi, dX, dY, dZ, tr, sTi, sdt, trc, Ntr, 100, fileName, bw, my_rank, shotNumber, 1, divisor); //Alterar dX, dY, dZ, sDivisor, Yi, Zi
     
        //Update propagation filename
 	sprintf(fileName, "retroP_%d.bin", shotNumber);

        propagation( &(vel[localGxMinC*Ybi*Zbi + localGyMinC*Zbi]), localXi, localYi, Yi, Zi, dX, dY, dZ, tracesData, sTi, sdt, tracesC, curTraceNumber, 100, fileName, bw, my_rank, shotNumber, -1, divisor);  //Alterar dX, dY, dZ, sDivisor, Yi, Zi
 ...
 */
void propagation(float *vel, unsigned int localXi, unsigned int localYi, int Yi, int Zi, float dx, float dy, float dz, float *tr, unsigned short Ti, float dt, int *trc, int Ntr, int wri, char *wffn, int bw, int my_rank, unsigned int shotNumber, short pMode, double divisor) {
    int xbi, ybi, zbi, ti, c, ntr, fIndex, cIndex; // auxiliary variables
        ...
        for (xbi = ncx-1; xbi < localXbi-ncx+1; xbi++) {
            for (ybi = ncy-1; ybi < localYbi-ncy+1; ybi++) {
                for (zbi = ncz-1; zbi < Zbi-ncz+1; zbi++) {
                    ...
                    ivb = xbi*Ybi*Zbi + ybi*Zbi + zbi;   //Alterar ivb (arredondar) 
        ...
        for (ntr = 0; ntr < Ntr; ntr++) 
	{
		...
		{
			//Repeat last traces
			fIndex = (Ti-divisor)/divisor;
			u.pn[indb] -= vel[ivb]*tr[ntr*Ti+fIndex];
		}
...
void getComShotTraces(FILE* suFile, SuTrace* traces, float* tracesData, int* tracesC, float* tr, int* trc, unsigned long* traceNumber, unsigned long* localTraceNumber, unsigned int* localXi, unsigned int* localYi, unsigned int* localGxMinC, unsigned int* localGyMinC, unsigned int gxMin, unsigned int gyMin, unsigned long ntt, unsigned short ns, unsigned int TD, float dX, float dY, float dZ, float dt)
{
	...
	//Invert data bytes 
        ...
        tracesC[curTraceNumber*3]   = (curGx-localGxMin)/dX;
	tracesC[curTraceNumber*3+1] = (curGy-localGyMin)/dY;
